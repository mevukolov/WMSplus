...
begin
  ...
  with candidates as (
    ...
    where t.enabled = true
      ...
      and coalesce(cardinality(t.weeek_assignee_ids), 0) = 0
      and (
...
comment on function public.assign_weeek_shift_task_assignees(text, date, boolean) is
  'Assigns uncompleted, unassigned WEEEK tasks to incoming/outgoing shift responsibles. AWH tasks are assigned from 3 days before due_date; other dated tasks are assigned when due_date is reached. Blocks current-day assignment after 20:00 Moscow unless explicitly overridden.';
