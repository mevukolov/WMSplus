...
type WeeekTag = {
  ...
  const TARGET_TABLE = "weeek_tasks_basic";
  ...
  const WEEEK_RATE_DELAY_MS = Math.min(Math.max(Number(Deno.env.get("WEEEK_RATE_DELAY_MS") ?? "350") || 350, 200), 1300);
  ...
  function normalizeTextList(value: unknown): string[] {
    ...
    return Array.from(new Set(normalizeText(value).split(",").map((item) => normalizeText(item)).filter(Boolean)));
  }
        ...
        if (Array.isArray(rawFields)) {
          for (const rawField of rawFields) {
            ...
            if ((fieldId && id === fieldId) || (expectedName && name === expectedName)) {
              const value = field.value ?? field.values ?? field.selectedValue ?? field.selected_value ?? field.option ?? field.selectedOption ?? field.selected_option;
...
type WeeekLocationMode = "locations" | "top_level";
  ...
  function buildWeeekPayload(row: WeeekTaskRow, config: WeeekConfig, target: { projectId: string; boardId: string | null; columnId: string }, locationMode: WeeekLocationMode = "locations"): JsonObject {
    ...
    if (row.priority !== null && row.priority !== undefined) {
      payload.priority = normalizeNumber(row.priority, 0);
    }
...
async function writeBackIncomingFlowResult(row: WeeekTaskRow, resolution: IncomingFlowResolution, config: WeeekConfig, timeoutMs: number): Promise<JsonObject> {
  ...
  async function countQueueRows(body: JsonObject, status: string | null, sourceModule: string | null): Promise<number> {
    const sourceModules = sourceModule ? [] : normalizeTextList(body.source_modules);
    let query = supabase
      .from(TARGET_TABLE)
    ...
    if (status) query = query.eq("master_status", status);
    else query = query.in("master_status", ["queued", "processing", "failed"]);

    if (sourceModule) query = query.eq("source_module", sourceModule);
    else if (sourceModules.length) query = query.in("source_module", sourceModules);

    const { count, error } = await query;
    if (error) throw new Error(`Failed to count ${TARGET_TABLE}: ${error.message}`);
    return count ?? 0;
  }

  async function queueSummary(body: JsonObject): Promise<JsonObject> {
    const sourceModules = normalizeTextList(body.source_modules);
    const singleSourceModule = normalizeText(body.source_module);
    const modules = sourceModules.length ? sourceModules : singleSourceModule ? [singleSourceModule] : [];
    const moduleSummaries: JsonObject[] = [];
    let queuedTotal = 0;
    ...
    if (modules.length) {
      for (const sourceModule of modules) {
        const queued = await countQueueRows(body, "queued", sourceModule);
        const processing = await countQueueRows(body, "processing", sourceModule);
        const failed = await countQueueRows(body, "failed", sourceModule);
        const remaining = queued + processing + failed;
        queuedTotal += queued;
        processingTotal += processing;
        failedTotal += failed;
        remainingTotal += remaining;
        moduleSummaries.push({ source_module: sourceModule, queued, processing, failed, remaining });
      }
    } else {
      queuedTotal = await countQueueRows(body, "queued", null);
      processingTotal = await countQueueRows(body, "processing", null);
      failedTotal = await countQueueRows(body, "failed", null);
      remainingTotal = queuedTotal + processingTotal + failedTotal;
    }
  ...
  Deno.serve(async (req) => {
    ...
    try {
      const noApiActions = ["list_queue", "queue", "queue_summary", "summary", "list_routes", "routes"];
      ...
      if (["queue_summary", "summary"].includes(action)) {
        const result = await queueSummary(body);
        return json(200, { ...result, started_at: startedAt, finished_at: new Date().toISOString() });
      }
